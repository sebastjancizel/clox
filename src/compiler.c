...
static void errorAt(Token *token, const char *message) {
  ...
  if (token->type == TOKEN_EOF) {
    ...
    // %.*s means print a string of length token->length, starting at
    // token->start
  }
...
static void binary(void) {
  ...
  parsePrecedence((Precedence)rule->precedence + 1); // +1 because we want to
...
// the table of rules for the Pratt parser
ParseRule rules[] = {
